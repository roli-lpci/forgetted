"""
incognito.cleaner — Session log finder and deleter.

After exiting incognito mode, the session log covering the incognito window
...
def find_session_log(session_id: str, agents_dir: str) -> Optional[Path]:
    ...
    for jsonl_file in root.rglob("*.jsonl"):
        if session_id in jsonl_file.stem:
            logger.debug("🕶️  Found session log: %s", jsonl_file)
            return jsonl_file
...
def delete_session_log(session_log: Path) -> bool:
    ...
    if not session_log.exists():
        logger.warning("🕶️  Session log not found: %s", session_log)
        return False
    ...
    try:
        ...
        logger.info("🕶️  Session log trashed: %s", session_log)
        return True
    except ImportError:
        pass
    except Exception as exc:
        logger.warning("🕶️  send2trash failed (%s), falling back to rename", exc)
    ...
    try:
        ...
        logger.info("🕶️  Session log renamed to %s", deleted_path)
        return True
    except OSError as exc:
        logger.error("🕶️  Failed to delete session log: %s", exc)
        return False
